...
一个面向个人开发者的 agent 研发工作流 kit。它把个人产品从构思、设计、实现、审查、测试、发布到复盘沉淀串成一套可复用流程。
...
业务代码修改必须先通过功能分支闸门、阶段闸门和并行开发隔离检查。涉及 UI 或前端的功能必须先完成 `/02B-UI设计`，或记录用户明确授权的范围有限设计豁免。文档分析和初始化不等于授权实现代码。
