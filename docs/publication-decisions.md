...
| 发布渠道 | GitHub / npm / 内部包仓 / tarball | GitHub + npm + tarball（v0.1.0 已授权目标） |
...
- 使用 Apache-2.0。
- 在 CI、tarball 和安装 smoke 全部通过后，从 clean reviewed commit 的仓库目录发布 npm `0.1.0`；公开核对 Registry `gitHead` / `dist.shasum` 后，再创建同 commit 的不可变 Git tag `v0.1.0` 和对应 GitHub Release。
