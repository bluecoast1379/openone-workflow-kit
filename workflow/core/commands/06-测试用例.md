...
测试用例：按风险输出正常、异常、边界、权限和回归检查，并选出应写入完成标准的验收项。
...
- **可直接验收**：每条用例必须绑定可重复验证方法（命令/脚本/测试名，或带步骤与判定标准的人工检查），能自动化时优先自动化；标注哪些应成为“必须通过”的验收项。
...
- 输出风险分级矩阵（用例 × 风险级 × 验证方法 × 是否验收项候选）；标注建议写入完成标准的必须验收项。
