...
- 合并或正式发布前必须升级为“完整检查”。先运行 `node workflow/core/tools/resolve-policy.cjs --workspace . --signal pre_merge_or_release --changed-files <path[,path...]>`，确认 `resolved_profile` 为 `strict`。
...
- 先读取仓库已有规则和 `team-profile.yaml#branch_model`；发布分支未检测或未配置时报告缺口，不猜测 `prod/main/test` 或自行创建发布分支。
...
- **发布条件**：完成标准中“必须通过”的验收项必须全部“已通过”，或已由用户确认跳过并记录风险。任何必须项为“未检查/未通过/改动后需重查”时，不得进入 `/09-发布执行`，应记录明确卡点。
...
- 在进入 `/09-发布执行` 前，记录所有会阻止发布的缺口。

## Suggested Structure

1. **发布范围**：本次发布包含和不包含的功能、修复、文档和配置。
2. **本地 Git 状态**：当前分支、基线、diff、未跟踪文件、是否需要 worktree。
3. **本地集成计划**：按已检测或已配置的仓库规则，说明源分支、集成分支、发布分支和 tag 步骤；未知项保持待确认。
